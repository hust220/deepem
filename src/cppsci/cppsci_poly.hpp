...
struct lstsq<::std::complex<double>> {
    ...
    lstsq(Array<::std::complex<double>> &x, Array<::std::complex<double>> y, double rcond) {
        ...
        c = array_sub(y, V<Vi>{{0, x.shape(1)}, {0, y.shape(1)}});
        s = s;
        rank = rank;
    }
};
...
struct polyfit {
    ...
    polyfit(const Arrayd &x, const Arrayd &y, Arrayi deg, const Arrayd &w = Arrayd{}, double rcond=-1) {
        init(x,y,deg,w,rcond);
    }

    polyfit(const Arrayd &x, const Arrayd &y, int deg, const Arrayd &w = Arrayd{}, double rcond=-1) {
