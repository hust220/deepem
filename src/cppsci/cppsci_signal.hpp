...
#include "cppsci_signal_convolve.hpp"
...
//            data.identify("data");
            array_take(output, i) = convolve(data, wavelet_data, CONVOLVE_SAME);
        }
    ...
    Arrayb boolrelextrema(_Array && data, _Comp && comparator, int axis=0, int order=1, int mode=ARRAY_TAKE_MODE_CLIP) {
        ...
        auto && main = array_takes(data, locs, axis, mode);
        for (int shift = 1; shift < order+1; shift++) {
            auto && plus = array_takes(data, locs + shift, axis, mode);
            auto && minus = array_takes(data, locs - shift, axis, mode);
    ...
    inline Arrayd array_cwt(const Arrayd &ms, const Arrayi &scales) {
        ...
        for (int i = 0; i < scales.size(); i++) {
            ...
            auto && sub = array_sub(wCoefs, V<Vi>{{i,i+1},{0,len*2}});
            for (int j = 0; j < len*2-1-m; j++) sub(j) = ff(j+m);
            for (int j = 0; j <= m; j++) sub(len*2-1-m+j) = ff(j);
        }
        return array_sub(wCoefs, V<Vi>{{0,scales.size()},{0,len}});
    }
    ...
    inline Arrayi getLocalMaximumCWT(const Arrayd &wCoefs, int ampTh=0) {
        ...
        for (int i = 0; i < wCoefs.shape(0); i++) {
            array_sub(localMax, V<Vi>{{i,i+1},{0,len}}) = localMaximum(array_sub(wCoefs, V<Vi>{{i,i+1},{0,len}}), 3);
        }
        JN_INFOA(localMax);
...
//        JN_INFOA(x1);
    ...
    inline double ridgeNoise(const Ridge &ridge, const Arrayd &wCoefs/*, const Arrayi &scales*/) {
        ...
        return quantile(array_sub(wCoefs, V<Vi>{{0, 1}, {start, end}}), 0.1);
    }
