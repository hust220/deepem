        ...
        Arrayd &output, int order, int mode, double cval)
{
    ...
    npy_intp idimensions[MAXDIM], istrides[MAXDIM];
    NI_Iterator io, ic;
    const double *matrix = matrix_ar.data();
    const double *shift = shift_ar.data();
...
//        pc = (void *)(PyArray_DATA(coordinates));
        pc = reinterpret_cast<const char *>(coordinates.data());
    }
...
//    pi = (void *)PyArray_DATA(input);
//    po = (void *)PyArray_DATA(output);
    pi = reinterpret_cast<const char *>(input.data());
    po = reinterpret_cast<char *>(output.data());
